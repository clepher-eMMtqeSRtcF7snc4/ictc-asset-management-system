...
export function ReorderTable({ data, showColumns }: { data: ReorderItem[]; showColumns: boolean }) {
      ...
      <div className="flex min-h-14 items-center justify-between gap-3 border-b px-4">
        ...
        {selected > 0 && (
          <div className="flex items-center gap-2 text-xs">
            ...
            <Button size="xs" variant="outline"><Plus /> New Reorder</Button>
          </div>
        )}
      </div>
        ...
        <Table>
          ...
          <TableBody>
            {table.getRowModel().rows.length ? (
              ...
              <TableRow>
                <TableCell colSpan={reorderColumns.length} className="h-36 text-center">
                  <p className="font-medium">No items found</p>
                  <p className="mt-1 text-sm text-muted-foreground">No reorder items match your current filters.</p>
                </TableCell>
              </TableRow>
            )}
          </TableBody>
