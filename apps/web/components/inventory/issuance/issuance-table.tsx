...
export function IssuanceTable({ data, showColumns }: { data: IssuanceRecord[]; showColumns: boolean }) {
      ...
      <div className="flex min-h-14 items-center justify-between gap-3 border-b px-4">
        ...
        {selected > 0 && (
          <div className="flex items-center gap-2 text-xs">
            ...
            <Button size="xs" variant="outline"><Plus /> New Issuance</Button>
          </div>
        )}
      </div>
        ...
        <Table>
          ...
          <TableBody>
            {table.getRowModel().rows.length ? (
              ...
              <TableRow>
                <TableCell colSpan={issuanceColumns.length} className="h-36 text-center">
                  <p className="font-medium">No records found</p>
                  <p className="mt-1 text-sm text-muted-foreground">No issuance records match your current filters.</p>
                </TableCell>
              </TableRow>
            )}
          </TableBody>
