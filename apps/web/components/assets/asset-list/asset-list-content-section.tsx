...
export function AssetListContentSection() {
  ...
  const filterStatus = (status?: AssetStatus) =>
    setFilters({ ...initialFilters, status: status ?? "ALL" });

  return (
    <main className="mx-auto max-w-[1440px] space-y-5">
      <AssetListHeaderSection />
      <AssetSummaryCards onFilter={filterStatus} />
      <AssetListToolbar
        filters={filters}
        onChange={setFilters}
        onReset={() => setFilters(initialFilters)}
        onColumns={() => setShowColumns((value) => !value)}
      />
      <AssetListTable data={data} showColumns={showColumns} />
    </main>
  );
}
