...
export const roomTypeListInputSchema = z
  .object({
    search: z.string().trim().optional(),
    page: z.number().int().min(1).optional(),
    pageSize: z.number().int().min(1).max(100).optional(),
...
export const roomTypeListOutputSchema = z.object({
  items: z.array(roomTypeSchema),
  total: z.number().int().nonnegative(),
  page: z.number().int().positive(),
  pageSize: z.number().int().positive(),
  totalPages: z.number().int().nonnegative(),
...
export type RoomTypeListInput = z.infer<typeof roomTypeListInputSchema>;
export type RoomTypeListOutput = z.infer<typeof roomTypeListOutputSchema>;
...
export const createRoomInputSchema = roomFieldSchema;
